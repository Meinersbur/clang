...
void EmitClangAttrParsedAttrImpl(RecordKeeper &Records, raw_ostream &OS) {
  ...
  OS << "static const ParsedAttrInfo "
        "AttrInfoMap[AttributeList::UnknownAttribute + 1] = {\n";
  OS << SS.str();
  OS << "};\n\n";
      ...
      for (const auto &S : Spellings) {
        ...
        if (SemaHandler)
          Matches->push_back(StringMatcher::StringPair(
              Spelling, "return AttributeList::AT_" + AttrName + ";"));
        else
          Matches->push_back(StringMatcher::StringPair(
              Spelling, "return AttributeList::IgnoredAttribute;"));
      }
    }
  }
...
static void WriteDocumentation(RecordKeeper &Records,
  ...
  OS << "   \"";
  if (Doc.SupportedSpellings & GNU)
    OS << "X";
  OS << "\",\"";
  if (Doc.SupportedSpellings & CXX11)
    OS << "X";
  OS << "\",\"";
  if (Doc.SupportedSpellings & C2x)
    OS << "X";
  OS << "\",\"";
  if (Doc.SupportedSpellings & Declspec)
    OS << "X";
  OS << "\",\"";
  if (Doc.SupportedSpellings & Keyword)
    OS << "X";
  OS << "\", \"";
  if (Doc.SupportedSpellings & Pragma)
    OS << "X";
  OS << "\", \"";
